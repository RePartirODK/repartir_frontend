...
/// Page de détails d'un mentoré avec possibilité de noter
class MentoreDetailPage extends StatefulWidget {
  final Map<String, dynamic> mentoring;

  const MentoreDetailPage({super.key, required this.mentoring});

  @override
  State<MentoreDetailPage> createState() => _MentoreDetailPageState();
}

class _MentoreDetailPageState extends State<MentoreDetailPage> {
  ...
  void initState() {
    super.initState();
    // Note actuelle du mentor sur ce jeune
    _selectedNote = widget.mentoring['noteMentor'] ?? 0;
  }
  ...
  Future<void> _attribuerNote() async {
    ...
    try {
      ...
      await _mentorService.noterJeune(mentoringId, _selectedNote);

      // Mettre à jour immédiatement la note dans le widget
      setState(() {
        widget.mentoring['noteMentor'] = _selectedNote;
      });
  ...
  Widget build(BuildContext context) {
    final prenom = (widget.mentoring['prenomJeune'] ?? '').toString().trim();
    final nom = (widget.mentoring['nomJeune'] ?? '').toString().trim();
    ...
    final urlPhotoJeune = (widget.mentoring['urlPhotoJeune'] ?? '').toString().trim();
                ...
                child: Column(
                  ...
                  children: <Widget>[
                    ...
                    if (objectif.isNotEmpty) ...[
                      _buildSectionTitle('Objectif', Icons.flag),
                    ...
                    _buildSectionTitle('Attribuer une note', Icons.grade),
                    const SizedBox(height: 15),
